...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      ...
      <div id="page-container">
        <div id="content-wrap">
          <div class="blockdiv">
            <h1 id="title">WELCOME TO VGR: VIDEO GAME REVIEWS</h1>
            <div class="label aboutus">
              <p id="motto">Our goal is to make your searching experience better and more fluid.</p>
            </div>
          </div>
          <div class="blockdiv">
            <h1>Statistics</h1>
            ...
            $numberofreviews = "SELECT count(*) FROM reviews";
            $reviewnor = $connect ->query($numberofreviews);
            if($reviewnor)
            {
              while($rows = $reviewnor -> fetch_array())
              {
                echo "<p class='mainli'>Reviews Written: {$rows[0]}<span id='reviewswritten'></span></p>";
              }
            }

            $numberofaccounts = "SELECT count(*) from users";
            $usersresult = $connect ->query($numberofaccounts);
            if($usersresult)
            {
              while($rowss = $usersresult -> fetch_array())
              {
                echo "<p class='mainli'>Accounts Registered: {$rowss[0]}<span id='accountsregistered'></span></p>";
              }
            }
            ?>  
            </div>
